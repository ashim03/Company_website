...
export function ProductShowcase({
  ...
  return (
    <VisualFrame className={cn("w-full", className)}>
      <div onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={(e) => { if (!e.currentTarget.contains(e.relatedTarget)) setPaused(false); }} className="relative flex min-h-[24rem] flex-col gap-5 p-6 sm:p-8 lg:grid lg:grid-cols-[1.15fr_0.85fr] lg:items-stretch">
        ...
        <div className="relative">
          <Link
            href={`/products/${focused.slug}`}
            className="group block w-full rounded-2xl gradient-border bg-card/90 p-5 text-left shadow-soft sm:p-6"
          >
            ...
            .map((p, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "absolute flex items-center gap-2.5 rounded-2xl border border-border/70 bg-card/90 px-4 py-3 shadow-card backdrop-blur transition-colors duration-300 hover:border-primary/50",
                    pos
                  )}
