...
(async()=>{
 const b=await chromium.launch({channel:'chrome'});const p=await b.newPage();const site=await b.newPage();p.setDefaultTimeout(20000);let original;
 ...
 try{
  ...
  await site.goto(base+'/contact');assert.equal(await site.locator('iframe[title="CodAstra Labs company location"]').count(),0);assert.equal(await site.locator('footer a[aria-label*="demo link"]').count(),0);console.log('Unpublished socials/map hidden and saved.');
